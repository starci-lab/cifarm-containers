    ...
    async harvestPlant(
        ...
        try {
            await mongoSession.withTransaction(async (session) => {
                ...
                const inventoryType = this.staticService.inventoryTypes.find((inventoryType) => {
                    switch (placedItemTile.plantInfo.plantType) {
                    case PlantType.Crop:
                        return inventoryType.crop?.toString() === plant.id
                    case PlantType.Flower:
                        return inventoryType.flower?.toString() === plant.id
                    }
                })
