...
import { SeedCommandOptions } from "./seed.types"
import { InjectDataSource } from "@nestjs/typeorm"
import { TEST_DATASOURCE_NAME } from "@src/dynamic-modules"
import { Command, CommandRunner, Option } from "nest-commander"
import { runInKubernetes } from "@src/utils"
import { Logger } from "@nestjs/common"


@Command({ name: "seed", description: "Seed static data into the database" })
export class SeedCommand extends CommandRunner {
    private readonly logger = new Logger(SeedCommand.name)

    constructor(
        private readonly dataSource: DataSource,
        @InjectDataSource(TEST_DATASOURCE_NAME)
    ...
    async run(_: Array<string>, options?: SeedCommandOptions): Promise<void> {
        if (!runInKubernetes()) {
            if (options?.test !== undefined && options?.test !== null) {
                await this.runWithTest()
                return
            }
        }
    ...
    async runDefault() {
        await this.seed(this.dataSource)
    }

    @Option({
        flags: "-t, --test",
        description: "Seed data into the test database (development only)",
    })
    async runWithTest() {
        await this.seed(this.testDataSource)
    }
