...
@Injectable()
export class RetrieveInventoriesService {
    private readonly logger = new Logger(RetrieveInventoriesService.name)
    ...
    async retrieveInventories(
        { id: userId }: UserLike,
        { inventoryIds }: RetrieveInventoriesMessage
        ...
        try {
            ...
            const result = await mongoSession.withTransaction(async (session) => {
                ...
                // Fetch the user's delivery inventories
                const deliveryInventories = await this.connection
                ...
                if (deliveryInventories.length !== inventoryIds.length) {
                ...
                // Create a new storage inventories
                for (const deliveryInventory of deliveryInventories) {
                    const inventoryType = this.staticService.inventoryTypes.find(
                        (inventoryType) => inventoryType.id === deliveryInventory.inventoryType.toString()
                    )
