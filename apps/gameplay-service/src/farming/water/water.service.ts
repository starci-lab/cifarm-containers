...
import { Activities, CropCurrentState, PlacedItemEntity, SystemEntity, SystemId, UserEntity } from "@src/database"
import { Cache } from "cache-manager"
import { DataSource } from "typeorm"
import { WaterRequest, WaterResponse } from "./water.dto"
import { EnergyNotEnoughException, PlacedItemTileNotFoundException, PlacedItemTileNotNeedWaterException, PlacedItemTileNotPlantedException } from "@src/exceptions"
...
export class WaterService {
    ...
    async water(request: WaterRequest): Promise<WaterResponse> {
        ...
        if (!placedItemTile)
            throw new PlacedItemTileNotFoundException(request.id)
        ...
        const { water : { energyCost, experiencesGain } } = value as Activities
        ...
        if (user.energy < energyCost)
            throw new EnergyNotEnoughException(user.energy, energyCost)
