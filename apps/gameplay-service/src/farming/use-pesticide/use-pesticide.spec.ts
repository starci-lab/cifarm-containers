// npx jest apps/gameplay-service/src/farming/use-pesticide/use-pesticide.spec.ts
...
describe("UsePesticideService", () => {
    ...
    beforeAll(async () => {
        const moduleRef = await Test.createTestingModule({
            ...
            providers: [UsePesticideService, EnergyService, LevelService],
        }).compile()
    ...
    it("should throw EnergyNotEnoughException when user does not have enough energy", async () => {
        ...
        const placedItemTile = await dataSource.manager.save(PlacedItemSchema, {
            ...
            userId: user.id,
            seedGrowthInfo: {
                currentState: CropCurrentState.IsInfested, // Not weedy
                currentStageTimeElapsed: 0,
                cropId: CropId.Carrot,
                harvestQuantityRemaining: 10,
            },
